...
# Skip if CN is a wildcard certificate (starts with *)
if [ "$CN" != "$HOST" ] && [ -n "$CN" ] && [[ "$CN" != \** ]]; then
...
LOOP_COUNT=$(grep -c "REDIRECT_LOOP" "$REDIRECT_LOOP_FILE" 2>/dev/null)
[ -z "$LOOP_COUNT" ] && LOOP_COUNT=0
EXCESSIVE_COUNT=$(grep -c "EXCESSIVE_REDIRECTS" "$REDIRECT_LOOP_FILE" 2>/dev/null)
[ -z "$EXCESSIVE_COUNT" ] && EXCESSIVE_COUNT=0
