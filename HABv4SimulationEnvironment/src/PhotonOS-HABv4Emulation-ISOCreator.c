...
static int create_secure_boot_iso(void) {
    ...
    /* Patch initrd to enable MOK package substitution during installation
     * This modifies the installer.py to detect photon.secureboot=mok and
     * substitute standard packages with MOK-signed variants */
    char initrd_path[512], initrd_extract[512], mok_patch_py[512];
    snprintf(initrd_path, sizeof(initrd_path), "%s/isolinux/initrd.img", iso_extract);
    snprintf(initrd_extract, sizeof(initrd_extract), "%s/initrd_extract", work_dir);
    snprintf(mok_patch_py, sizeof(mok_patch_py), "%s/mok_patch.py", work_dir);
    
    if (file_exists(initrd_path)) {
        log_info("Patching initrd for MOK package substitution...");
        
        /* Extract initrd */
        mkdir_p(initrd_extract);
        snprintf(cmd, sizeof(cmd), 
            "cd '%s' && zcat '%s' 2>/dev/null | cpio -idm --no-absolute-filenames 2>/dev/null",
            initrd_extract, initrd_path);
        run_cmd(cmd);
        
        /* Create mok_patch.py module */
        FILE *f = fopen(mok_patch_py, "w");
        if (f) {
            fprintf(f,
                "#!/usr/bin/env python3\n"
                "\"\"\"\n"
                "MOK Secure Boot Package Substitution for Photon OS Installer\n"
                "Auto-generated by PhotonOS-HABv4Emulation-ISOCreator\n"
                "\"\"\"\n"
                "import os\n"
                "import shlex\n"
                "\n"
                ...
                "        return packages\n"
                "    msg = '[MOK-PATCH] MOK secure boot mode - applying package substitutions'\n"
                "    if logger:\n"
                "        logger.info(msg)\n"
                "    else:\n"
                "        print(msg)\n"
                ...
                "        if base_pkg in MOK_PACKAGE_SUBSTITUTIONS:\n"
                "            new_pkg = MOK_PACKAGE_SUBSTITUTIONS[base_pkg]\n"
                "            msg = f'[MOK-PATCH] Substituting: {pkg} -> {new_pkg}'\n"
                "            if logger:\n"
                "                logger.info(msg)\n"
                "            else:\n"
                "                print(msg)\n"
                "            substituted.add(base_pkg)\n"
                ...
                "                if mok == pkg and orig not in substituted:\n"
                "                    msg = f'[MOK-PATCH] Adding: {pkg}'\n"
                "                    if logger:\n"
                "                        logger.info(msg)\n"
                "                    else:\n"
                "                        print(msg)\n"
                "                    new_packages.append(pkg)\n"
                "                    break\n"
                "    return list(dict.fromkeys(new_packages))\n"
            );
            fclose(f);
        }
        
        /* Copy mok_patch.py to initrd */
        char initrd_installer_dir[512];
        snprintf(initrd_installer_dir, sizeof(initrd_installer_dir), 
            "%s/usr/lib/python3.11/site-packages/photon_installer", initrd_extract);
        if (dir_exists(initrd_installer_dir)) {
            snprintf(cmd, sizeof(cmd), "cp '%s' '%s/mok_patch.py'", mok_patch_py, initrd_installer_dir);
            run_cmd(cmd);
            
            /* Patch installer.py using sed - add import and modify package assignment */
            char installer_py[512];
            snprintf(installer_py, sizeof(installer_py), "%s/installer.py", initrd_installer_dir);
            
            if (file_exists(installer_py)) {
                /* Check if already patched */
                snprintf(cmd, sizeof(cmd), "grep -q 'mok_patch' '%s'", installer_py);
                if (run_cmd(cmd) != 0) {
                    /* Add import after 'import tdnf' */
                    snprintf(cmd, sizeof(cmd),
                        "sed -i '/^import tdnf/a\\\n"
                        "try:\\\n"
                        "    from mok_patch import apply_mok_substitution\\\n"
                        "except ImportError:\\\n"
                        "    apply_mok_substitution = lambda p, l=None: p' '%s'",
                        installer_py);
                    run_cmd(cmd);
                    
                    /* Modify the package assignment to apply MOK substitution */
                    snprintf(cmd, sizeof(cmd),
                        "sed -i \"s/install_config\\['packages'\\] = packages_pruned/"
                        "install_config['packages'] = apply_mok_substitution(packages_pruned, getattr(self, 'logger', None))/g\" '%s'",
                        installer_py);
                    run_cmd(cmd);
                    
                    log_info("installer.py patched for MOK package substitution");
                } else {
                    log_info("installer.py already patched");
                }
            }
            ...
            snprintf(cmd, sizeof(cmd),
                "cd '%s' && find . | cpio -o -H newc 2>/dev/null | gzip -9 > '%s.new' && mv '%s.new' '%s'",
                initrd_extract, initrd_path, initrd_path, initrd_path);
            if (run_cmd(cmd) == 0) {
                log_info("initrd repacked with MOK patch");
            } else {
                log_warn("Failed to repack initrd - MOK package substitution may not work");
            }
        } else {
            log_warn("Could not find installer directory in initrd");
        }
