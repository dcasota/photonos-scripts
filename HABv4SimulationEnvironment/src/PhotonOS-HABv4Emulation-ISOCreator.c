    ...
    if (have_installer_py) {
        ...
        if (pf) {
            fprintf(pf,
                ...
                "\n"
                "# Fix 5 (v1.9.39): rewrite _adjust_packages_based_on_selected_flavor\n"
                "#  to correctly handle multi-hyphen flavor names like linux-esx-mok.\n"
                "#  The original logic does package.split('-')[1] and matches against a\n"
                "#  list of flavor SECOND-TOKENS — which is fine for linux-mok / linux-esx\n"
                "#  but mishandles linux-esx-mok: its second token 'esx' collides with the\n"
                "#  unselected linux-esx, causing the installer to DROP linux-esx-mok from\n"
                "#  install_config['packages'] when the user picks it. The user reaches\n"
                "#  the partition screen, runs the install, and tdnf reports the MOK\n"
                "#  kernel as missing.\n"
                "#\n"
                "#  Fix: store the FULL suffix after 'linux-' in redundant_linux_flavors,\n"
                "#  and in filter_packages compare exact suffix match OR <suffix>-<dep>\n"
                "#  where dep is in self.linux_dependencies (devel/drivers/docs/oprofile/dtb).\n"
                "#  This catches linux-esx-devel (drop when esx is redundant) but keeps\n"
                "#  linux-esx-mok (because 'mok' is not in linux_dependencies).\n"
                "#\n"
                "#  See docs/plans/installer-ui-permutation-cadastre.md §4(f) for the\n"
                "#  full analysis of the tdnf-side failure mode this prevents.\n"
                "before5 = content\n"
                "content = re.sub(\n"
                "    r'        def filter_packages\\(package\\):\\n'\n"
                "    r'            package = package\\.split\\(\\'-\\'\\)\\n'\n"
                "    r'            if len\\(package\\) > 1:\\n'\n"
                "    r'                flavor = package\\[1\\]\\n'\n"
                "    r'            else:\\n'\n"
                "    r'                flavor = \"\"\\n'\n"
                "    r'            if \\(package\\[0\\] != \"linux\"\\):\\n'\n"
                "    r'                return True\\n'\n"
                "    r'            elif \\(\"\" in redundant_linux_flavors and flavor in self\\.linux_dependencies\\):\\n'\n"
                "    r'                return False\\n'\n"
                "    r'            elif \\(flavor in redundant_linux_flavors\\):\\n'\n"
                "    r'                return False\\n'\n"
                "    r'            else:\\n'\n"
                "    r'                return True',\n"
                "    '        def filter_packages(package):\\n'\n"
                "    '            # v1.9.39: full-suffix match so multi-hyphen flavors work\\n'\n"
                "    '            if not (package == \"linux\" or package.startswith(\"linux-\")):\\n'\n"
                "    '                return True\\n'\n"
                "    '            pkg_suffix = package[6:] if package.startswith(\"linux-\") else \"\"\\n'\n"
                "    '            # Exact match on a full flavor suffix → drop\\n'\n"
                "    '            if pkg_suffix in redundant_linux_flavors:\\n'\n"
                "    '                return False\\n'\n"
                "    '            # Sub-package of a redundant flavor (linux-<suffix>-<dep>)\\n'\n"
                "    '            for _redundant in redundant_linux_flavors:\\n'\n"
                "    '                if _redundant:\\n'\n"
                "    '                    for _dep in self.linux_dependencies:\\n'\n"
                "    '                        if pkg_suffix == _redundant + \"-\" + _dep:\\n'\n"
                "    '                            return False\\n'\n"
                "    '                else:\\n'\n"
                "    '                    if pkg_suffix in self.linux_dependencies:\\n'\n"
                "    '                        return False\\n'\n"
                "    '            return True',\n"
                "    content, count=1)\n"
                "if content == before5:\n"
                "    print('WARN: filter_packages body not found in installer.py - filter fix skipped')\n"
                "\n"
                "# Fix 6 (v1.9.39): rewrite the redundant_linux_flavors construction to\n"
                "#  store the FULL suffix (everything after 'linux-') instead of just\n"
                "#  the second hyphen-separated token. Required by the Fix 5 filter.\n"
                "before6 = content\n"
                "content = re.sub(\n"
                "    r'        for flavor in self\\.all_linux_flavors:\\n'\n"
                "    r'            if \\(flavor != self\\.install_config\\[\\'linux_flavor\\'\\]\\):\\n'\n"
                "    r'                flavor = flavor\\.split\\(\\'-\\'\\)\\n'\n"
                "    r'                if len\\(flavor\\) > 1:\\n'\n"
                "    r'                    flavor = flavor\\[1\\]\\n'\n"
                "    r'                else:\\n'\n"
                "    r'                    flavor = \"\"\\n'\n"
                "    r'                redundant_linux_flavors\\.append\\(flavor\\)',\n"
                "    '        for flavor in self.all_linux_flavors:\\n'\n"
                "    '            if flavor != self.install_config[\\'linux_flavor\\']:\\n'\n"
                "    '                # v1.9.39: store full suffix (e.g. \"esx-mok\") not just \"esx\"\\n'\n"
                "    '                suffix = flavor[6:] if flavor.startswith(\"linux-\") else \"\"\\n'\n"
                "    '                redundant_linux_flavors.append(suffix)',\n"
                "    content, count=1)\n"
                "if content == before6:\n"
                "    print('WARN: redundant_linux_flavors loop not found - construction fix skipped')\n"
                "\n"
                "# Fix 8 (v1.9.43 — whitelist mok_quickstart in Installer.known_keys):\n"
                "#   v1.9.41 added install_config['mok_quickstart']='generic|esx' as the\n"
                "#   cross-screen sentinel (MokQuickstartSelector writes it; PackageSelector,\n"
                "#   LinuxSelector, StigEnable all read it). But Installer._check_install_config\n"
                "#   at installer.py:558-560 validates install_config.keys() against the class\n"
                "#   constant known_keys at lines 65-114 and raises:\n"
                "#       InstallerConfigError: Unknown install_config keys: mok_quickstart\n"
                "#   on the install path. Operator hit this AFTER picking Yes-Generic in v1.9.42.\n"
                "#\n"
                "#   Workflow w25w25dbl recommended Option A: whitelist mok_quickstart in the\n"
                "#   known_keys set (matches existing pattern for linux_flavor + packages +\n"
                "#   ansible + additional_packages — all top-level keys written by screens).\n"
                "#   Option B (pop before validate) was rejected because it breaks kickstart\n"
                ...
                "\n"
                "# v1.9.58 (cadastre concern #15 / edge-case finding #30 — install-blocker):\n"
                "#   The v1.9.57 back-nav fix added `install_config['_quickstart_committed']`\n"
                "#   as the pop-on-display sentinel. On the LAST forward visit (operator\n"
                "#   commits + never back-walks) display() is never invoked again, so the\n"
                "#   flag survives into Installer._check_install_config which raises\n"
                "#   InstallerConfigError: Unknown install_config keys: {_quickstart_committed}.\n"
                "#   v1.9.58 extends the v1.9.56 monkey-patch pattern to also whitelist\n"
                "#   '_quickstart_committed'. Belt-and-suspenders with the\n"
                "#   iso_config.configure() pop landing in the iso_config patcher below.\n"
                "monkey_patch_v1958 = (\n"
                "    '\\n# Appended by HABv4 ISOCreator v1.9.58 — belt-and-suspenders whitelist of _quickstart_committed\\n'\n"
                "    '# (cadastre edge-case finding #30: terminal-commit path bypasses MokQuickstart.display() pop)\\n'\n"
                "    'try:\\n'\n"
                "    '    if isinstance(Installer.known_keys, set):\\n'\n"
                "    '        Installer.known_keys.add(\"_quickstart_committed\")\\n'\n"
                "    '    elif isinstance(Installer.known_keys, (list, tuple)):\\n'\n"
                "    '        if \"_quickstart_committed\" not in Installer.known_keys:\\n'\n"
                "    '            Installer.known_keys = list(Installer.known_keys) + [\"_quickstart_committed\"]\\n'\n"
                "    '    elif isinstance(Installer.known_keys, frozenset):\\n'\n"
                "    '        Installer.known_keys = Installer.known_keys | {\"_quickstart_committed\"}\\n'\n"
                "    'except (NameError, AttributeError):\\n'\n"
                "    '    pass\\n'\n"
                ")\n"
                "if 'HABv4 ISOCreator v1.9.58' not in content:\n"
                "    content = content + monkey_patch_v1958\n"
                "    print('v1.9.58 _quickstart_committed monkey-patch appended to installer.py')\n"
                "else:\n"
                ...
                "#   — destroying the original exception. The operator sees only\n"
                "#   'InstallerError: Installer failed' with no diagnostic detail.\n"
                "#   Fix: extend exit_gracefully signature with cause=None; chain via\n"
                "#   `raise InstallerError(f'Installer failed: {cause!r}') from cause`\n"
                "#   so the original exception text is surfaced AND tracebacks chain.\n"
                "#   Then update the 3 callers that have an exception in scope.\n"
                "before7 = content\n"
                "\n"
                "# 7a: extend exit_gracefully signature\n"
                "content = re.sub(\n"
                "    r'def exit_gracefully\\(self,\\s*signal1=None,\\s*frame1=None\\):',\n"
                "    'def exit_gracefully(self, signal1=None, frame1=None, cause=None):',\n"
                "    content, count=1)\n"
                "\n"
                "# 7b: chain the cause through the raise at the end of exit_gracefully\n"
                "content = re.sub(\n"
                "    r\"        raise InstallerError\\(\\\"Installer failed\\\"\\)\",\n"
                "    '        if cause is not None:\\n'\n"
                "    '            raise InstallerError(f\"Installer failed: {cause!r}\") from cause\\n'\n"
                "    '        raise InstallerError(\"Installer failed\")',\n"
                "    content, count=1)\n"
                "\n"
                "# 7c: pass the caught exception into exit_gracefully at the _install\n"
                "#     site (installer.py:776-778). Use a regex tolerant of whitespace.\n"
                "content = re.sub(\n"
                "    r'(except Exception as inst:\\s*\\n'\n"
                "    r'\\s*self\\.logger\\.exception\\(repr\\(inst\\)\\)\\s*\\n'\n"
                "    r'\\s*)self\\.exit_gracefully\\(\\)',\n"
                "    r'\\1self.exit_gracefully(cause=inst)',\n"
                "    content, count=1)\n"
                "\n"
                "if content == before7:\n"
                "    print('WARN: exit_gracefully cause-chaining not applied (v1.9.40 Fix 7)')\n"
                "\n"
                "with open(sys.argv[1], 'w') as f:\n"
                "    f.write(content)\n"
                "\n"
                "print('Patch applied successfully (progress_bar + all_linux_flavors + filter_packages + cause-chain + mok_quickstart-known-key)')\n"
            );
            fclose(pf);
    ...
    if (have_linuxselector_py) {
        ...
        if (pf) {
            fprintf(pf,
                ...
                "    content = f.read()\n"
                "\n"
                "# FIX 1 (v1.9.38): Add linux-mok + linux-esx-mok to linux_flavors.\n"
                "before = content\n"
                "content = re.sub(\n"
                ...
                "# 'linux-esx' string; once linux-esx-mok ships an RPM, this row\n"
                "# would appear on bare-metal/KVM/QEMU where the ESX-tuned kernel\n"
                "# does not belong.\n"
                "before2 = content\n"
                "content = re.sub(\n"
                "    r'if flavor == \"linux-esx\" and not CommandUtils\\.is_vmware_virtualization\\(\\):',\n"
                "    'if flavor in (\"linux-esx\", \"linux-esx-mok\") and not CommandUtils.is_vmware_virtualization():',\n"
                "    content, count=1)\n"
                "if content == before2:\n"
                "    print('WARN: hypervisor gate not found in linuxselector.py - widening skipped')\n"
                "\n"
                "# FIX 3 (v1.9.39 — cadastre Rec #2; v1.9.40 — collector bug repaired):\n"
                "# guard the zero-menu crash. linuxselector.py originally constructs\n"
                "# Menu(...) + Window(...) + set_action_panel(...) unconditionally.\n"
                "# When menu_items is empty (e.g. all flavors filtered by the\n"
                "# hypervisor gate on bare-metal/KVM, or the new MOK quickstart\n"
                "# auto-skip in v1.9.41), Menu's __init__ divides by zero. The\n"
                "# display()-level `len < 2 -> inactive_screen` guard is dead code\n"
                "# for the zero case because Menu/Window are built first. We wrap\n"
                "# the entire Menu+Window+set_action_panel block in an if-guard.\n"
                "#\n"
                "# v1.9.40 collector bug repair: the v1.9.39 line-based collector\n"
                "# terminated on the BLANK LINE between Menu(...) and Window(...),\n"
                "# leaving Window+set_action_panel OUTSIDE the if-guard. The booted\n"
                "# v1.9.39 ISO therefore still crashed with `AttributeError: ...\n"
                "# has no attribute 'host_menu'` whenever len(menu_items)<2 because\n"
                "# set_action_panel(self.host_menu) ran with host_menu unassigned.\n"
                "# The regex path is also dropped (it never matched the multi-line\n"
                "# Photon 5.0 source with the blank line); always run the state-\n"
                "# machine collector that terminates on set_action_panel.\n"
                "lines = content.split('\\n')\n"
                "out = []\n"
                "i = 0\n"
                "patched = False\n"
                "while i < len(lines):\n"
                "    line = lines[i]\n"
                "    if (not patched and 'self.host_menu = Menu(' in line\n"
                "            and 'self.menu_starty' in line):\n"
                "        # State machine: collect lines until set_action_panel.\n"
                "        # Blank lines, continuation lines, Window(...) all\n"
                "        # included verbatim until the terminator is found.\n"
                "        block = [line]\n"
                "        j = i + 1\n"
                "        while j < len(lines):\n"
                "            block.append(lines[j])\n"
                "            if 'self.window.set_action_panel(' in lines[j]:\n"
                "                break  # last line of block — done\n"
                "            j += 1\n"
                "            if j - i > 12:\n"
                "                # safety: bail if we haven't found the terminator\n"
                "                # within 12 lines (the actual block is 6-7 lines).\n"
                "                print('WARN: set_action_panel terminator not found '\n"
                "                      'within 12 lines of Menu(self.menu_starty) — '\n"
                "                      'patch skipped, install will crash on empty menu')\n"
                "                block = []\n"
                "                break\n"
                "        if block:\n"
                "            # indent each collected line by 4 extra spaces, prepend the guard\n"
                "            indent = ' ' * (len(line) - len(line.lstrip()))\n"
                "            out.append(indent + 'if len(self.menu_items) >= 2:')\n"
                "            for b in block:\n"
                "                out.append('    ' + b)\n"
                "            i = j + 1\n"
                "            patched = True\n"
                ...
                "    content = '\\n'.join(out)\n"
                "else:\n"
                "    print('WARN: zero-menu Menu construction not found - guard skipped')\n"
                "    sys.exit(3)\n"
                "\n"
                "# v1.9.40 post-write indent assertion (cadastre+plan recommendation):\n"
                "# fail-fast if set_action_panel is still at the outer 8-space indent\n"
                "# instead of the guarded 12-space indent. This would have caught the\n"
                "# v1.9.39 collector bug at build time instead of in the operator's VM.\n"
                "for verify_line in content.split('\\n'):\n"
                "    if 'self.window.set_action_panel(self.host_menu)' in verify_line:\n"
                "        leading = len(verify_line) - len(verify_line.lstrip())\n"
                "        if leading < 12:\n"
                "            print('FATAL: FIX 3 guard incomplete - set_action_panel at '\n"
                "                  'indent ' + str(leading) + ', expected >=12 '\n"
                "                  '(inside the if-guard). Empty-menu case will still AttributeError.')\n"
                "            sys.exit(4)\n"
                "        break\n"
                "\n"
                "with open(sys.argv[1], 'w') as f:\n"
                "    f.write(content)\n"
                "\n"
                "print('linuxselector.py patched: linux-(esx-)mok dict + widened ESX gate + zero-menu guard (v1.9.40 complete)')\n"
            );
            fclose(pf);
    ...
    if (have_installer_py) {
        ...
        if (mq) {
            fprintf(mq,
                ...
                "        # v1.9.57: record that operator made a fresh commit on this\n"
                "        # screen, so display()'s Q3 guard can distinguish a forward\n"
                "        # auto-skip (committed=True) from a back-nav re-render\n"
                "        # (committed=False, operator may want to change their mind).\n"
                "        self.install_config['mok_quickstart'] = 'no'\n"
                "        self.install_config['_quickstart_committed'] = True\n"
                "        # v1.9.58 (cadastre concern #31/32 — Yes->back->No symmetry):\n"
                "        # a previous _apply_yes pre-filled ansible/additional_packages/\n"
                "        # packages/linux_flavor. If we don't clear them, StigEnable's\n"
                "        # v1.9.57 ansible-guard short-circuits the No-path and the\n"
                "        # operator who explicitly opted OUT of MOK still gets a\n"
                "        # STIG-hardened install with no UI signal. Clear them so the\n"
                "        # downstream screens re-render and re-capture operator intent.\n"
                "        # NOTE: 'ostree' is intentionally NOT restored — _apply_yes\n"
                "        # popped it deliberately; restoring would conflict with the\n"
                "        # No path's normal OSTree-question chain. Documented in\n"
                "        # cadastre concern #34 as an accepted asymmetry.\n"
                ...
                "\n"
                "    def display(self):\n"
                "        # Q3 (ADR-0027): pre-set — skip the screen.\n"
                "        # v1.9.57 back-nav fix (operator bug report — workflow\n"
                "        # wbm4o1r5r Bug C): emit the canonical inactive_screen\n"
                "        # sentinel (NOT ActionResult(True, None)) so that\n"
                "        # iso_config.configure()'s preserve-direction guard\n"
                "        #   if ar.result and ar.result.get('inactive_screen', False)\n"
                "        # keeps the back-walk going BACKWARD instead of bouncing\n"
                "        # forward. _quickstart_committed is set by set_no/_apply_yes\n"
                "        # (forward commit) and by iso_config's kickstart-aware seed\n"
                "        # (kickstart pre-seed). We pop it so that a Back -> re-render\n"
                "        # round-trip surfaces the screen again (operator can flip Yes\n"
                "        # to No or vice versa).\n"
                "        if self.install_config.get('mok_quickstart') in ('no', 'generic', 'esx') \\\n"
                "                and self.install_config.pop('_quickstart_committed', False):\n"
                "            return ActionResult(None, {'inactive_screen': True})\n"
        ...
        if (file_exists(iso_cfg_path)) {
            ...
            if (pf) {
                fprintf(pf,
                    ...
                    "#\n"
                    "# v1.9.57 back-nav fix (operator bug report — workflow\n"
                    "# wbm4o1r5r Bug C): can_go_back flag flipped False -> True\n"
                    "# so the iso_config.configure() back-walk inner loop\n"
                    "#   while index >= 0 and items[index][1] is False: index -= 1\n"
                    "# no longer skips PAST MokQuickstart. MokQuickstartSelector\n"
                    "# now emits an inactive_screen sentinel when forward auto-\n"
                    "# skip is desired (kickstart pre-seed or committed Yes/No),\n"
                    "# so flipping to True does NOT regress forward auto-skip.\n"
                    "before2 = content\n"
                    "content = re.sub(\n"
                    "    r'(\\n        )(items\\.append\\(\\(package_selector\\.display, True\\)\\))',\n"
                    "    r'\\1mok_quickstart_selector = MokQuickstartSelector(maxy, maxx, install_config)\\1items.append((mok_quickstart_selector.display, True))\\1\\2',\n"
                    "    content, count=1)\n"
                    "if content == before2:\n"
                    "    print('WARN: package_selector.display append not found - screen not inserted')\n"
                    "    sys.exit(3)\n"
                    "\n"
                    "# v1.9.57 kickstart-aware committed flag: when install_config\n"
                    "# arrives with mok_quickstart already keyed (kickstart JSON\n"
                    "# pre-seed), pre-set _quickstart_committed so the MokQuick-\n"
                    "# startSelector.display() Q3 guard auto-skips on first\n"
                    "# render (preserves the kickstart auto-skip critical path).\n"
                    "# Anchored on the MokQuickstartSelector instantiation we\n"
                    "# just inserted; only seeds when a kickstart pre-seed is\n"
                    "# present, so manual-install paths are untouched.\n"
                    "before3 = content\n"
                    "content = re.sub(\n"
                    "    r'(\\n        mok_quickstart_selector = MokQuickstartSelector\\(maxy, maxx, install_config\\))',\n"
                    "    r\"\\n        # v1.9.57 kickstart-aware committed flag\\n\"\n"
                    "    r\"        if install_config.get('mok_quickstart') in ('no', 'generic', 'esx'):\\n\"\n"
                    "    r\"            install_config['_quickstart_committed'] = True\\1\",\n"
                    "    content, count=1)\n"
                    "if content == before3:\n"
                    "    print('WARN: v1.9.57 kickstart-aware committed flag not inserted')\n"
                    "    sys.exit(4)\n"
                    "\n"
                    "# v1.9.58 (cadastre edge-case finding #30 — install-blocker, A1b):\n"
                    "#   Pop install_config['_quickstart_committed'] at the end of\n"
                    "#   IsoConfig.configure() before it returns. The UI-state flag\n"
                    "#   must NOT survive into installer.py's _check_install_config\n"
                    "#   (which would raise Unknown install_config keys).\n"
                    "#   Belt-and-suspenders with the v1.9.58 monkey-patch in\n"
                    "#   installer.py — even if the monkey-patch is reverted by a\n"
                    "#   future patcher regression, the pop here keeps the flag\n"
                    "#   UI-scoped where it belongs.\n"
                    "#   Anchor: the `return install_config` line at the end of\n"
                    "#   IsoConfig.configure(). Idempotent: skip if the v1.9.58\n"
                    "#   marker already present.\n"
                    "before4 = content\n"
                    "if 'v1.9.58 configure-pop' not in content:\n"
                    "    content = re.sub(\n"
                    "        r'(\\n        )(return install_config\\b)',\n"
                    "        r\"\\1# v1.9.58 configure-pop: scope _quickstart_committed to UI only\\1install_config.pop('_quickstart_committed', None)\\1\\2\",\n"
                    "        content, count=1)\n"
                    "    if content == before4:\n"
                    "        print('WARN: v1.9.58 configure-pop anchor (return install_config) not found')\n"
                    "        sys.exit(5)\n"
                    "    else:\n"
                    "        print('v1.9.58 configure-pop inserted before return install_config')\n"
                    "else:\n"
                    "    print('v1.9.58 configure-pop already present (idempotent)')\n"
                    "\n"
                    "with open(sys.argv[1], 'w') as f:\n"
                    "    f.write(content)\n"
                    "print('iso_config.py patched: MokQuickstartSelector inserted (v1.9.57 back-nav + v1.9.58 configure-pop)')\n"
                );
                fclose(pf);
        ...
        if (file_exists(stig_path)) {
            ...
            if (pf) {
                fprintf(pf,
                    ...
                    "\n"
                    "# v1.9.57 back-nav fix (operator bug report — workflow\n"
                    "# wbm4o1r5r Bug C, half 1): the v1.9.41/42 idiom EXTENDED\n"
                    "# the ostree guard to also check 'ansible', but its return\n"
                    "# value was 'ActionResult(True, None)' — NOT the canonical\n"
                    "# 'ActionResult(None, {\"inactive_screen\": True})' sentinel\n"
                    "# that iso_config.configure()'s preserve-direction guard\n"
                    "#   if ar.result and ar.result.get('inactive_screen', False)\n"
                    "# keys off. So on a back-walk, ar.success stayed True and\n"
                    "# the index bounced FORWARD instead of continuing backward.\n"
                    "# Fix: insert an EXPLICIT inactive_screen short-circuit\n"
                    "# ABOVE the ostree guard when 'ansible' is pre-seeded,\n"
                    "# matching the PackageSelector + LinuxSelector idiom.\n"
                    "# v1.9.42 marker for idempotent re-runs\n"
                    "MARKER = '# v1.9.57-stigenable-inactive-screen'\n"
                    "if MARKER in content:\n"
                    "    print('[patch_stigenable] v1.9.57 already applied')\n"
                    "    sys.exit(0)\n"
                    "\n"
                    "# If a previous v1.9.41/42 'and ansible not in' guard is\n"
                    "# present, revert it to the upstream single-condition form\n"
                    "# first, so the v1.9.57 anchor matches deterministically.\n"
                    "content = re.sub(\n"
                    "    r\"if \\('ostree' not in self\\.install_config\\s*\\n\"\n"
                    "    r\"\\s*and 'ansible' not in self\\.install_config\\):\\n\"\n"
                    ...
                    "    content, count=1)\n"
                    "if content == before:\n"
                    "    print('FATAL: StigEnable display() anchor not found - back-nav fix NOT installed')\n"
                    "    sys.exit(2)\n"
                    "\n"
                    "with open(sys.argv[1], 'w') as f:\n"
                    "    f.write(content)\n"
                    ...
                    "verify = open(sys.argv[1]).read()\n"
                    "\n"
                    "# A5: new inactive_screen guard MUST appear BEFORE the ostree check\n"
                    "ansi_pos = verify.find(\"'ansible' in self.install_config\")\n"
                    "ostree_pos = verify.find(\"'ostree' not in self.install_config\")\n"
                    "if ansi_pos == -1 or ostree_pos == -1 or ansi_pos > ostree_pos:\n"
                    "    print('FATAL A5: ansible inactive_screen guard not above ostree guard')\n"
                    "    sys.exit(3)\n"
                    "\n"
                    "# A6: sentinel form MUST match upstream PackageSelector idiom\n"
                    "if \"{'inactive_screen': True}\" not in verify:\n"
                    "    print('FATAL A6: inactive_screen sentinel literal not present')\n"
                    "    sys.exit(4)\n"
                    "\n"
                    "print('stigenable.py patched v1.9.57: explicit inactive_screen sentinel + post-conditions OK')\n"
                );
                fclose(pf);
        ...
        if (cfg.efuse_usb_mode) {
            ...
            if (epf) {
                fprintf(epf,
                    ...
                    "efuse_code = '''\n"
                    "# HABv4 eFuse USB Verification (v1.9.59 auto-retry, v1.9.65 hard-security)\n"
                    "#\n"
                    "# v1.9.65 ROOT-CAUSE FIX for eFuse-not-detected on MBR + UEFI:\n"
                    "# The stock mk-setup-grub.sh only does `insmod ext2 part_gpt`. Without\n"
                    "# `insmod fat`, GRUB's search --label cannot scan FAT-labeled volumes,\n"
                    "# which is exactly what EFUSE_SIM is (FAT32 filesystem). The marker\n"
                    "# file lives on a FAT32 partition, so search silently returns\n"
                    "# 'no such device' regardless of whether the device is physically\n"
                    "# accessible.\n"
                    "#\n"
                    "# `insmod part_msdos` covers MBR-partitioned disks (some USB sticks\n"
                    "# are partitioned MBR rather than GPT; floppies have no partition\n"
                    ...
                    "fi\n"
                    "\n"
                    "# v1.9.65 HARD SECURITY ENFORCEMENT:\n"
                    "# After 3 failed attempts, show a Retry/Shutdown menu and BLOCK boot.\n"
                    "# DO NOT fall through to the kernel menuentry without verified eFuse.\n"
                    "#\n"
                    "# Why hard security: the prior chainloader fallback on MBR (where\n"
                    "# /EFI/BOOT/grubx64.efi does not exist) silently failed and execution\n"
                    "# fell through to the menuentry below -- kernel booted WITHOUT\n"
                    "# eFuse verified, violating the HABv4 security model. The operator\n"
                    "# observed dracut emergency afterwards, but that was an accidental\n"
                    "# safety net -- a malicious or misconfigured kernel could have run.\n"
                    "#\n"
                    "# Menu is rendered in the themed gfxterm (font from grub2-theme is\n"
                    "# loaded by the stock grub.cfg loadfont section before this block).\n"
                    "if [ \"\\\\$efuse_verified\" = \"0\" ]; then\n"
                    "    echo \"\"\n"
                    "    echo \"============================================================\"\n"
                    "    echo \"  HABv4 SECURITY: eFuse marker NOT detected\"\n"
                    "    echo \"============================================================\"\n"
                    "    echo \"\"\n"
                    "    echo \"GRUB cannot find a block device labeled EFUSE_SIM with\"\n"
                    "    echo \"/efuse_sim/srk_fuse.bin on it.\"\n"
                    "    echo \"\"\n"
                    "    echo \"Verify VM hardware:\"\n"
                    "    echo \"  - UEFI VMs: attach the .vmdk descriptor (NOT raw .img)\"\n"
                    "    echo \"             as SATA disk. UEFI does not expose floppy.\"\n"
                    "    echo \"  - BIOS VMs: SATA via .vmdk is the canonical attachment.\"\n"
                    "    echo \"             Floppy attachment may fail if .img > 2.88 MB.\"\n"
                    "    echo \"  - Check 'Connect at power on' in VM Settings.\"\n"
                    "    echo \"\"\n"
                    "    echo \"This system will NOT boot without verified eFuse.\"\n"
                    "    set timeout=-1\n"
                    "    menuentry \"Retry - Reboot to rescan devices\" {\n"
                    "        reboot\n"
                    "    }\n"
                    "    menuentry \"Shutdown\" {\n"
                    "        halt\n"
                    "    }\n"
                    "    # Fail-safe: if we ever reach the end of this block without\n"
                    "    # operator selection, halt unconditionally. GRUB normally waits\n"
                    "    # at timeout=-1, but this protects against weird script flow.\n"
                    "    halt\n"
                    "fi\n"
                    "\n"
