    ...
    if (have_installer_py) {
        ...
        if (pf) {
            fprintf(pf,
                ...
                "    f.write(content)\n"
                "\n"
                "print('Patch applied successfully (progress_bar + all_linux_flavors + filter_packages)')\n"
            );
            fclose(pf);
    ...
    if (have_linuxselector_py) {
        ...
        if (pf) {
            fprintf(pf,
                ...
                "\n"
                "# FIX 3 (v1.9.39 — cadastre Rec #2): guard the zero-menu crash.\n"
                "# linuxselector.py originally constructs Menu(...) unconditionally\n"
                "# at the end of create_available_linux_menu(). When menu_items is\n"
                "# empty (e.g. all flavors filtered by the hypervisor gate), Menu's\n"
                "# __init__ divides by len(items)=0 at menu.py:41 -> ZeroDivisionError.\n"
                "# The display()-level `len < 2 -> inactive_screen` guard is dead\n"
                "# code for the zero case because Menu is built first. Wrap the\n"
                "# Menu/Window construction in an if-guard so empty menus skip them.\n"
                "before3 = content\n"
                "content = re.sub(\n"
                "    r'(\\n        )(self\\.host_menu = Menu\\(self\\.menu_starty[^)]*\\)[^\\n]*\\n[ ]*[^\\n]*\\n[ ]*[^\\n]*\\n[ ]*[^\\n]*\\n[ ]*[^\\n]*\\n[ ]*self\\.window\\.set_action_panel\\(self\\.host_menu\\))',\n"
                "    r'\\1if len(self.menu_items) >= 2:\\n            \\2'.replace('\\n            ', '\\n            '),\n"
                "    content, count=1)\n"
                "# The re.sub above is fragile because the source spans 6 lines with\n"
                "# specific indentation. Fall back to a more reliable line-based\n"
                "# transform if the regex did not match:\n"
                "if content == before3:\n"
                "    lines = content.split('\\n')\n"
                "    out = []\n"
                "    i = 0\n"
                ...
                "            # Collect this and the following Window-construction lines.\n"
                "            block = [line]\n"
                "            j = i + 1\n"
                "            while j < len(lines) and (\n"
                "                    lines[j].strip().startswith('default_selected')\n"
                "                    or lines[j].strip().startswith('tab_enable')\n"
                "                    or lines[j].strip().startswith('self.window =')\n"
                "                    or lines[j].strip().startswith(\"'Select Linux kernel\")\n"
                "                    or lines[j].strip().startswith('position=')\n"
                "                    or lines[j].strip().startswith('can_go_next=')\n"
                "                    or lines[j].strip().startswith('self.window.set_action_panel(')):\n"
                "                block.append(lines[j])\n"
                "                j += 1\n"
                "            # Indent the block by 4 more spaces, prepend the guard.\n"
                ...
                "    f.write(content)\n"
                "\n"
                "print('linuxselector.py patched: linux-(esx-)mok dict + widened ESX gate + zero-menu guard')\n"
            );
            fclose(pf);
