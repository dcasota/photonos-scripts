...
static int generate_grub_mok_spec(rpm_build_config_t *config, rpm_package_info_t *grub_pkg) {
    ...
    fprintf(f,
        ...
        "# regardless of version differences between GRUB builds\n"
        ...
        "# SUSE shim looks for grub.efi, so install under both names\n"
        "cp %%{keys_dir}/grub-mok.efi ./boot/efi/EFI/BOOT/grub.efi\n"
        "\n"
        "%%build\n"
        ...
        "# SUSE shim looks for grub.efi (not grubx64.efi)\n"
