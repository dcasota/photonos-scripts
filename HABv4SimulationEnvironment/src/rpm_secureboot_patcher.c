...
static int generate_grub_mok_spec(rpm_build_config_t *config, rpm_package_info_t *grub_pkg) {
    ...
    fprintf(f,
        ...
        "Distribution:   Photon\n"
        "\n"
        "# Provides same capability as original and obsoletes it\n"
...
static int generate_linux_mok_spec(rpm_build_config_t *config, rpm_package_info_t *linux_pkg) {
    ...
    fprintf(f,
        ...
        "\n"
        "# Provides same capability as original and obsoletes it\n"
        "# Using Obsoletes (without version) ensures MOK package replaces original\n"
        "# regardless of version differences between kernel builds\n"
        "Provides:   %%{linux_name} = %%{linux_version}-%%{linux_release}%%{?dist}\n"
        "Provides:   linux = %%{linux_version}-%%{linux_release}%%{?dist}\n"
        "Provides:   linux-esx = %%{linux_version}-%%{linux_release}%%{?dist}\n"
        ...
        "\n"
        "%%post\n"
        "# Create kernel module dependencies\n"
        "/sbin/depmod -a %%{linux_version}-%%{linux_release}\n"
        "# For -esx kernel, the release includes the flavor suffix\n"
        "test -f /boot/linux-%%{linux_version}-%%{linux_release}.cfg && \\\n"
        "  ln -sf linux-%%{linux_version}-%%{linux_release}.cfg /boot/photon.cfg\n"
        "# Also try with flavor suffix (for linux-esx style kernels)\n"
        "KVER=\"\"\n"
        "for cfg in /boot/linux-%%{linux_version}-*.cfg; do\n"
        "  if [ -f \"$cfg\" ]; then\n"
        "    ln -sf \"$(basename $cfg)\" /boot/photon.cfg\n"
        "    KVER=$(basename \"$cfg\" .cfg | sed 's/linux-//')\n"
        "    break\n"
        "  fi\n"
        "done\n"
        "# Generate initrd if dracut is available and initrd doesn't exist\n"
        "# Include USB drivers for USB boot support (ESX kernel has these as modules)\n"
        "if [ -n \"$KVER\" ] && [ -x /usr/sbin/dracut ]; then\n"
        "  if [ ! -f \"/boot/initrd.img-$KVER\" ]; then\n"
        "    echo \"Generating initrd for kernel $KVER with USB drivers...\" >&2\n"
        "    # Rebuild module dependencies first\n"
        "    /sbin/depmod -a \"$KVER\" 2>/dev/null || true\n"
        "    # Generate initrd with USB drivers for USB boot support\n"
        "    /usr/sbin/dracut -f \\\n"
        "      --add-drivers \"usbcore usb-common xhci_hcd xhci_pci ehci_hcd ehci_pci uhci_hcd usb_storage\" \\\n"
        "      \"/boot/initrd.img-$KVER\" \"$KVER\" 2>/dev/null || \\\n"
        "    /usr/sbin/dracut -f \"/boot/initrd.img-$KVER\" \"$KVER\" 2>/dev/null || true\n"
        "  fi\n"
        "fi\n"
        "\n"
        "%%postun\n"
        ...
        "fi\n"
        "\n"
        "# Trigger to mark initrd generation as pending when initramfs package is installed\n"
        "%%triggerin -- initramfs\n"
        "mkdir -p /var/lib/rpm-state/initramfs/pending\n"
        "# Get kernel version from the installed .cfg file\n"
        "for cfg in /boot/linux-%%{linux_version}-*.cfg; do\n"
        "  if [ -f \"$cfg\" ]; then\n"
        "    kver=$(basename \"$cfg\" .cfg | sed 's/linux-//')\n"
        "    touch \"/var/lib/rpm-state/initramfs/pending/$kver\"\n"
        "    echo \"initrd generation of kernel $kver will be triggered later\" >&2\n"
        "    break\n"
        "  fi\n"
        "done\n"
        "\n"
