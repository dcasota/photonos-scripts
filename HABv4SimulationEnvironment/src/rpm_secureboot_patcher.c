...
static int generate_grub_mok_spec(rpm_build_config_t *config, rpm_package_info_t *grub_pkg) {
    ...
    fprintf(f,
        ...
        "Distribution:   Photon\n"
        "\n"
        "# Provides same capability as original and conflicts with it\n"
...
static int generate_linux_mok_spec(rpm_build_config_t *config, rpm_package_info_t *linux_pkg) {
    ...
    fprintf(f,
        ...
        "\n"
        "# Provides same capability as original and conflicts with it\n"
        "# Using Conflicts instead of versioned Obsoletes ensures MOK package\n"
        "# cannot be installed alongside ANY version of the original packages.\n"
        "# This is necessary because custom kernel builds may have older version numbers\n"
        "# than the ISO's kernel (e.g., 6.1.159 MOK vs 6.12.60 original).\n"
        ...
        "    \n"
        "    # Find the extracted vmlinuz file to replace (use head -1 to handle multiple files)\n"
        ...
        "             nvme nvme_core virtio_blk virtio_scsi virtio_pci virtio_net \\\n"
        "             hv_vmbus hv_storvsc hv_netvsc; do\n"
        ...
        "\n"
        "%%install\n"
        "# Install kernel boot files (excluding /boot/efi which belongs to grub2-efi-image)\n"
        ...
        "fi\n"
        "\n"
        "# v1.9.70 D5: touch /.autorelabel on the target so SELinux relabels late-\n"
        "# created files (/etc/group, /etc/hostname) on first boot. Without this,\n"
        "# auditd_t cannot read /etc/group (no SELinux label) -> getgrnam(\"root\")\n"
        "# returns NULL -> auditd fails with \"Group ID is non-numeric and unknown\n"
        "# (root)\". Standard Photon SELinux first-boot relabel.\n"
        "touch /.autorelabel\n"
        "echo \"linux-mok v1.9.70 D5: touched /.autorelabel for first-boot SELinux relabel\"\n"
        "\n"
        "# v1.9.71 D2: write /etc/dracut.conf.d/usb-boot.conf on the target so the\n"
        "# next dracut regenerate (next kernel update or `dracut -f`) includes\n"
        "# the USB boot driver set as force_drivers. v1.9.64 wrote this into the\n"
        "# installer initrd only; Photon installer does not copy /etc/dracut.conf.d\n"
        "# from installer rootfs to target. Restore commit 7d04098 (Jan 28, 2026).\n"
        "mkdir -p /etc/dracut.conf.d\n"
        "cat > /etc/dracut.conf.d/usb-boot.conf <<'USBBOOT_EOF'\n"
        "# v1.9.71 HABv4 patch (commit 7d04098 restore, Jan 28, 2026):\n"
        "# force_drivers ensures USB modules are unconditionally loaded at early boot.\n"
        "# Without this, kernel may not see USB-attached root devices in time.\n"
        "force_drivers+=\" usbcore usb-common xhci_hcd xhci_pci ehci_hcd ehci_pci uhci_hcd usb_storage \"\n"
        "USBBOOT_EOF\n"
        "echo \"linux-mok v1.9.71 D2: created /etc/dracut.conf.d/usb-boot.conf on target\"\n"
        "\n"
        "%%postun\n"
...
static int generate_linux_esx_mok_spec(rpm_build_config_t *config,
    ...
    fprintf(f,
        ...
        "# (root)\". Standard Photon SELinux first-boot relabel.\n"
        "touch /.autorelabel\n"
        "echo \"linux-esx-mok v1.9.70 D5: touched /.autorelabel for first-boot SELinux relabel\"\n"
        "\n"
        "# v1.9.71 D2: write /etc/dracut.conf.d/usb-boot.conf on the target so the\n"
        "# next dracut regenerate (next kernel update or `dracut -f`) includes\n"
        "# the USB boot driver set as force_drivers. v1.9.64 wrote this into the\n"
        ...
        "force_drivers+=\" usbcore usb-common xhci_hcd xhci_pci ehci_hcd ehci_pci uhci_hcd usb_storage \"\n"
        "USBBOOT_EOF\n"
        "echo \"linux-esx-mok v1.9.71 D2: created /etc/dracut.conf.d/usb-boot.conf on target\"\n"
        "\n"
        "%%postun\n"
        "if [ ! -e /boot/photon.cfg ]; then\n"
        "  list=\"$(basename \"$(ls -1 -tu /boot/linux-*.cfg 2>/dev/null | head -n1)\")\"\n"
