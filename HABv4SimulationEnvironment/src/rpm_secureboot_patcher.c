...
static int generate_linux_mok_spec(rpm_build_config_t *config, rpm_package_info_t *linux_pkg) {
    ...
    fprintf(f,
        ...
        "\n"
        "%%post\n"
        "# Determine the full kernel version string including flavor suffix\n"
        "# For linux-esx: vmlinuz-6.1.159-7.ph5-esx -> KVER=6.1.159-7.ph5-esx\n"
        "# For linux: vmlinuz-6.1.159-7.ph5 -> KVER=6.1.159-7.ph5\n"
        "KVER=\"\"\n"
        ...
        "done\n"
        "\n"
        "# Fallback if no vmlinuz found\n"
        "if [ -z \"$KVER\" ]; then\n"
        "  # Try to detect from modules directory\n"
        ...
        "\n"
        "if [ -n \"$KVER\" ]; then\n"
        "  echo \"linux-mok: Detected kernel version: $KVER\"\n"
        "  \n"
        ...
        "  # This is critical - without initrd the system won't boot!\n"
        "  echo \"linux-mok: Generating initrd for $KVER\"\n"
        "  if [ -x /usr/bin/dracut ]; then\n"
        "    /usr/bin/dracut -f --kver \"$KVER\" \\\n"
        "      --add-drivers \"xhci_pci ehci_pci uhci_hcd usb_storage sd_mod\" \\\n"
        "      \"/boot/initrd.img-${KVER}\" 2>&1 || echo \"linux-mok: WARNING: dracut failed\"\n"
        "    if [ -f \"/boot/initrd.img-${KVER}\" ]; then\n"
        "      echo \"linux-mok: Created /boot/initrd.img-${KVER}\"\n"
        "    else\n"
        "      echo \"linux-mok: ERROR: initrd was not created!\"\n"
        "    fi\n"
        "  else\n"
        "    echo \"linux-mok: WARNING: dracut not found, initrd not generated\"\n"
        "  fi\n"
        "else\n"
