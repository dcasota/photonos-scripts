...
static int generate_linux_mok_spec(rpm_build_config_t *config, rpm_package_info_t *linux_pkg) {
    ...
    fprintf(f,
        ...
        "    # It should be in $KERNEL_BUILD_DIR/linux-*/modules/lib/modules/<version>/\n"
        "    # We iterate to find it.\n"
        "    FOUND_MODULES=0\n"
        "    for mod_path in \"$KERNEL_BUILD_DIR\"/linux-*/modules/lib/modules/*; do\n"
        "        if [ -d \"$mod_path\" ]; then\n"
        ...
        "\n"
        "%%post\n"
        "# Determine the correct kernel version for depmod\n"
        "# Priority: 1. vmlinuz filename IF matching modules exist\n"
        "#           2. Any existing modules directory matching the version\n"
        "KVER=\"\"\n"
        "for vmlinuz in /boot/vmlinuz-%%{linux_version}-*; do\n"
        "  if [ -f \"$vmlinuz\" ]; then\n"
        "    TEMP_KVER=$(basename \"$vmlinuz\" | sed 's/vmlinuz-//')\n"
        "    if [ -d \"/lib/modules/$TEMP_KVER\" ]; then\n"
        "       KVER=$TEMP_KVER\n"
        "       break\n"
        "    fi\n"
        "  fi\n"
        ...
        "\n"
        "if [ -z \"$KVER\" ]; then\n"
        "  # Fallback: look for any module directory starting with version\n"
        "  # This handles the case where vmlinuz name (e.g. -esx) doesn't match modules (custom build)\n"
        ...
        "\n"
        "if [ -n \"$KVER\" ]; then\n"
        "  # Create kernel module dependencies with CORRECT version\n"
        "  echo \"linux-mok: Running depmod -a $KVER\"\n"
        "  /sbin/depmod -a \"$KVER\"\n"
        "  \n"
        "  # Create photon.cfg symlink\n"
        "  if [ -f \"/boot/linux-${KVER}.cfg\" ]; then\n"
        "    ln -sf \"linux-${KVER}.cfg\" /boot/photon.cfg\n"
        "    echo \"linux-mok: Created /boot/photon.cfg -> linux-${KVER}.cfg\"\n"
        "  fi\n"
        "  # Initrd is already installed by the package, no need to generate it here\n"
        "else\n"
