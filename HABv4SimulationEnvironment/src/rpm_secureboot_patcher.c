...
static int generate_grub_mok_spec(rpm_build_config_t *config, rpm_package_info_t *grub_pkg) {
    ...
    fprintf(f,
        ...
        "# Using Obsoletes instead of Conflicts allows smooth replacement when\n"
        "# minimal meta-package requires grub2-efi-image\n"
        "Provides:   grub2-efi-image = %%{grub_version}-%%{grub_release}%%{?dist}\n"
        "Obsoletes:  grub2-efi-image < %%{grub_version}-%%{grub_release}.mok1%%{?dist}\n"
        "\n"
        "# Note: grub2-theme dependency removed - no package provides it in Photon OS ISO\n"
        "# The theme files are included in the ISO boot environment, not needed for installed system\n"
        "\n"
        "%%description\n"
...
static int generate_linux_mok_spec(rpm_build_config_t *config, rpm_package_info_t *linux_pkg) {
    ...
    fprintf(f,
        ...
        "\n"
        "# Provides same capability as original and obsoletes it\n"
        "# Using Obsoletes allows minimal meta-package dependency resolution\n"
        ...
        "Obsoletes:  %%{linux_name} < %%{linux_version}-%%{linux_release}.mok1%%{?dist}\n"
        "Obsoletes:  linux-esx < %%{linux_version}-%%{linux_release}.mok1%%{?dist}\n"
        "\n"
...
static int generate_shim_mok_spec(rpm_build_config_t *config, 
    ...
    fprintf(f,
        ...
        "\n"
        "# Provides same capability as original and obsoletes it\n"
        "# Using Obsoletes allows smooth replacement during installation\n"
        "Provides:   shim-signed = %%{shim_version}-%%{shim_release}%%{?dist}\n"
        "Obsoletes:  shim-signed < %%{shim_version}-%%{shim_release}.mok1%%{?dist}\n"
        "\n"
        "%%description\n"
