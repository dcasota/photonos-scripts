...
#
# This script deploys Powershell v7.1.0-preview.4 on VMware Photon OS. To start Powershell simply enter "pwsh7.1.0.p4".
#
#
# History
...
# There is no differenciation of "Core" anymore. However, Powershell Core on Linux does not contain those modules which are made available through the Microsoft Windows Compatibility Pack on 64-bit Windows.
# Cmdlets in nettcpip and in other net* modules are not cross-platform yet. With each Powershell release more and more functionality is implemented, potentially deliverable on Photon, too.
#
# On Photon 'tndf install -y powershell' latest release is 7.0.0.
# This script downloads and installs Powershell 7.1.0-preview.4 release.
#    Powershell is installed in /opt/microsoft/powershell/7.1.0-preview.4/ with a symbolic link "pwsh7.1.0.p4" that points to /opt/microsoft/powershell/7.1.0-preview.4/pwsh.
# 
#
...
export PS_INSTALL_VERSION=7.1.0.p4
