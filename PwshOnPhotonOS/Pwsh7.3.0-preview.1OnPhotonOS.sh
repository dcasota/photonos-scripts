#!/bin/sh
# Deploy Powershell v7.3.0-preview.1 on VMware Photon OS
#
# This script deploys Powershell v7.3.0-preview.1 on VMware Photon OS. To start Powershell simply enter "pwsh7.3.0-preview.1".
#
#
# History
# 0.1  24.01.2022   dcasota  Initial release
#
# Prerequisites:
...
#
# Description:
# 'tndf install -y powershell' installs a Photon OS release specific build on x86_x64.
# 
# As alternative, this script downloads and does a scripted install of Powershell Core pwsh7.3.0-preview.1. It also works on arm64 (tested on Pi4).
#
# See release info https://github.com/PowerShell/PowerShell/releases/tag/v7.3.0-preview.1
# See blog about PowerShell 7.0 https://devblogs.microsoft.com/powershell/announcing-powershell-7-0/.
#
# Powershell is installed in /opt/microsoft/powershell/7.3.0-preview.1/ with a symbolic link "pwsh7.3.0-preview.1" that points to /opt/microsoft/powershell/7.3.0-preview.1/pwsh.
#
#    Especially when running on Photon OS 2.0, two workarounds are necessary to be saved in profile /opt/microsoft/powershell/7.3.0-preview.1/profile.ps1.
#       Without those you might run into following issues:
#       find-module VMware.PowerCLI
#       Find-Package: /opt/microsoft/powershell/7.3.0-preview.1/Modules/PowerShellGet/PSModule.psm1
#       Line |
...
#    
#       Each time pwsh7.3.0-preview.1 is started the saved profile with the workarounds is loaded.
...
#  curl -O -J -L https://raw.githubusercontent.com/dcasota/photonos-scripts/master/Pwsh7.1.5OnPhotonOS.sh
#  sudo chmod a+x ./Pwsh7.3.0-preview.1OnPhotonOS.sh
#  sudo ./Pwsh7.3.0-preview.1OnPhotonOS.sh
#
...
export PS_VERSION=7.3.0-preview.1
export PACKAGE_VERSION=7.3.0-preview.1
export PS_PACKAGE=powershell-${PACKAGE_VERSION}-linux-x64.tar.gz
if (uname -m | grep -q "aarch64"); then
	export PS_PACKAGE=powershell-${PACKAGE_VERSION}-linux-arm64.tar.gz
fi
...
export PS_INSTALL_VERSION=7.3.0-preview.1
