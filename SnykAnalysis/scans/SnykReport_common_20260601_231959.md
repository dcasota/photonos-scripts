# Snyk Issues Report (common)

Generated: 06/02/2026 01:20:18
Total source packages processed: 1
Tool used: SnykCLI

## Summary

| Metric | Count |
|---|---|
| Total issues overall | 8 |
| High with crypto | 0 |
| High without crypto | 0 |
| Medium | 8 |
| Low | 0 |

## Top 10 High Categories

(none)



## Top 10 Source Packages with most issues (with breakdown)

| Package | TotalIssues | HighCrypto | High | Medium | Low |
| --- | --- | --- | --- | --- | --- |
| stalld | 8 | 0 | 0 | 8 | 0 |


## Top 10 Source Packages with most issues on Level "High (+ crypto)"

(none)


## Top 10 Source Packages with most issues on Level "High"

(none)


## Top 10 Source Packages with most issues on Level "Medium"

| Package | Count |
| --- | --- |
| stalld | 8 |


## Top 10 Source Packages with most issues on Level "Low"

(none)


## Processing status (branch common)

| Step | Detail |
|---|---|
| SAST scan | ok (1 subdirs) |
| Agent-scan coverage | configs found=0, scans run=0, failed=0 |


