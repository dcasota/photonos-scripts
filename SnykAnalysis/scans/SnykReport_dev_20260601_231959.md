# Snyk Issues Report (dev)

Generated: 06/02/2026 01:20:21
Total source packages processed: 695
Tool used: SnykCLI

## Summary

| Metric | Count |
|---|---|
| Total issues overall | 56919 |
| High with crypto | 193 |
| High without crypto | 720 |
| Medium | 21627 |
| Low | 34379 |

## Top 10 High Categories

| Category | Count |
| --- | --- |
| Use of a Broken or Risky Cryptographic Algorithm | 140 |
| Cross-site Scripting (XSS) | 127 |
| Potential Negative Number Used as Index | 116 |
| Path Traversal | 107 |
| Hardcoded Secret | 90 |
| Command Injection | 62 |
| Hardcoded Non-Cryptographic Secret | 48 |
| Generation of Error Message Containing Sensitive Information | 38 |
| Size Used as Index | 35 |
| XML External Entity (XXE) Injection | 33 |


### Top 10 Packages with HIGH issues in category "Use of a Broken or Risky Cryptographic Algorithm" (category total=140)

| Package | Count |
| --- | --- |
| jdk21u | 54 |
| jdk11u | 54 |
| runtime | 19 |
| pycryptodome | 3 |
| podman | 2 |
| common | 2 |
| apr-util | 2 |
| apr | 2 |
| mysql-server | 1 |
| cryptography | 1 |

### Top 10 Packages with HIGH issues in category "Cross-site Scripting (XSS)" (category total=127)

| Package | Count |
| --- | --- |
| tomcat | 90 |
| kapacitor | 15 |
| uwsgi | 4 |
| freetds | 4 |
| zookeeper | 2 |
| gevent | 2 |
| tornado | 1 |
| runtime | 1 |
| rabbitmq-server | 1 |
| kubernetes | 1 |

### Top 10 Packages with HIGH issues in category "Potential Negative Number Used as Index" (category total=116)

| Package | Count |
| --- | --- |
| llvm-project | 35 |
| wireshark | 6 |
| redis | 6 |
| server | 5 |
| mysql-server | 5 |
| node | 4 |
| ntp | 3 |
| libxcrypt | 3 |
| systemtap | 2 |
| ruby | 2 |

### Top 10 Packages with HIGH issues in category "Path Traversal" (category total=107)

| Package | Count |
| --- | --- |
| tomcat | 40 |
| ruby | 18 |
| podman | 8 |
| govmomi | 7 |
| telegraf | 6 |
| zookeeper | 4 |
| runtime | 3 |
| go | 3 |
| common | 3 |
| src | 2 |

### Top 10 Packages with HIGH issues in category "Hardcoded Secret" (category total=90)

| Package | Count |
| --- | --- |
| jdk11u | 44 |
| jdk21u | 43 |
| tomcat | 1 |
| runtime | 1 |
| kafka | 1 |

### Top 10 Packages with HIGH issues in category "Command Injection" (category total=62)

| Package | Count |
| --- | --- |
| go | 19 |
| podman | 8 |
| common | 7 |
| termshark | 4 |
| etcd | 4 |
| containerd | 4 |
| moby | 3 |
| libcap | 3 |
| heapster | 2 |
| tomcat | 1 |

### Top 10 Packages with HIGH issues in category "Hardcoded Non-Cryptographic Secret" (category total=48)

| Package | Count |
| --- | --- |
| python | 28 |
| tornado | 5 |
| node | 5 |
| rabbitmq-server | 2 |
| uwsgi | 1 |
| pyzmq | 1 |
| pydantic | 1 |
| pycryptodome | 1 |
| psutil | 1 |
| gevent | 1 |

### Top 10 Packages with HIGH issues in category "Generation of Error Message Containing Sensitive Information" (category total=38)

| Package | Count |
| --- | --- |
| go | 11 |
| podman | 6 |
| plugins | 6 |
| kubernetes | 6 |
| common | 6 |
| dataplaneapi | 2 |
| etcd | 1 |

### Top 10 Packages with HIGH issues in category "Size Used as Index" (category total=35)

| Package | Count |
| --- | --- |
| mysql-server | 17 |
| src | 3 |
| protobuf | 3 |
| leveldb | 3 |
| llvm-project | 2 |
| kexec-tools | 2 |
| u-boot | 1 |
| pmd-next-gen | 1 |
| libqmi | 1 |
| geos | 1 |

### Top 10 Packages with HIGH issues in category "XML External Entity (XXE) Injection" (category total=33)

| Package | Count |
| --- | --- |
| xmlsec | 14 |
| xerces-c | 7 |
| libxslt | 6 |
| tomcat | 4 |
| runtime | 1 |
| libxml2 | 1 |


## Top 10 Source Packages with most issues (with breakdown)

| Package | TotalIssues | HighCrypto | High | Medium | Low |
| --- | --- | --- | --- | --- | --- |
| jdk21u | 2997 | 54 | 105 | 727 | 2165 |
| jdk11u | 2855 | 54 | 105 | 785 | 1965 |
| node | 2837 | 5 | 22 | 429 | 2386 |
| llvm-project | 2828 | 0 | 39 | 450 | 2339 |
| runtime | 2012 | 20 | 28 | 240 | 1744 |
| subversion | 1471 | 0 | 0 | 49 | 1422 |
| mysql-server | 1343 | 1 | 27 | 608 | 708 |
| wireshark | 1143 | 0 | 6 | 170 | 967 |
| src | 1004 | 0 | 6 | 708 | 290 |
| openssh-portable | 939 | 0 | 0 | 363 | 576 |


## Top 10 Source Packages with most issues on Level "High (+ crypto)"

| Package | Count |
| --- | --- |
| jdk21u | 54 |
| jdk11u | 54 |
| python | 28 |
| runtime | 20 |
| tornado | 5 |
| node | 5 |
| pycryptodome | 4 |
| unbound | 2 |
| rabbitmq-server | 2 |
| podman | 2 |


## Top 10 Source Packages with most issues on Level "High"

| Package | Count |
| --- | --- |
| tomcat | 142 |
| jdk21u | 105 |
| jdk11u | 105 |
| llvm-project | 39 |
| go | 34 |
| runtime | 28 |
| python | 28 |
| podman | 28 |
| mysql-server | 27 |
| node | 22 |


## Top 10 Source Packages with most issues on Level "Medium"

| Package | Count |
| --- | --- |
| jdk11u | 785 |
| jdk21u | 727 |
| src | 708 |
| aufs-linux | 654 |
| mysql-server | 608 |
| cups | 592 |
| util-linux | 493 |
| server | 478 |
| crash | 467 |
| llvm-project | 450 |


## Top 10 Source Packages with most issues on Level "Low"

| Package | Count |
| --- | --- |
| node | 2386 |
| llvm-project | 2339 |
| jdk21u | 2165 |
| jdk11u | 1965 |
| runtime | 1744 |
| subversion | 1422 |
| wireshark | 967 |
| zsh | 737 |
| mysql-server | 708 |
| openssh-portable | 576 |


## Processing status (branch dev)

| Step | Detail |
|---|---|
| SAST scan | ok (693 subdirs) |
| Agent-scan coverage | configs found=18, scans run=0, failed=0 |

## Agent Components (snyk-agent-scan)

| Metric | Count |
|---|---|
| Scans | 19 |
| Packages scanned | 18 |
| Total issues | 0 |
| High / Critical | 0 |
| Medium | 0 |
| Low / Info | 0 |

### Coverage by agent type

| AgentType | Packages | Issues |
| --- | --- | --- |
| claude | 9 | 0 |
| gemini | 10 | 0 |


### Top 10 Agent-Component Issue Codes

(none)


### Top 10 Packages with Agent-Component Issues

(none)

