# Snyk Issues Report (master)

Generated: 05/11/2026 12:07:06
Total source packages processed: 0
Tool used: SnykCLI

## Summary

| Metric | Count |
|---|---|
| Total issues overall |  |
| High with crypto |  |
| High without crypto |  |
| Medium |  |
| Low |  |

## Top 10 High Categories

(none)



## Top 10 Source Packages with most issues (with breakdown)

(none)


## Top 10 Source Packages with most issues on Level "High (+ crypto)"

(none)


## Top 10 Source Packages with most issues on Level "High"

(none)


## Top 10 Source Packages with most issues on Level "Medium"

(none)


## Top 10 Source Packages with most issues on Level "Low"

(none)


## Processing status (branch master)

| Step | Detail |
|---|---|
| SAST scan | `/root/actions-runner/_work/photonos-scripts/photonos-scripts/reports/photon-upstreams/photon-master/clones` empty — skipped |
| Agent-scan coverage | configs found=0, scans run=0, failed=0 |


