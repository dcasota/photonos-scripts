...
| TBD  | **M31** — ADR-0014 impl: cols 13/14 SHA256Name/SHA512Name schema | PS + C + parity-diff + diff_analyzer. New `pr_sha_of_url_multi` (single GET, dual hash). pr_state_t grows two fields. Schema growth gated by env var `PR_EMIT_MULTI_SHA` (default off → 12-col matches cached snapshot). Operator regenerates PS snapshot with env var on for cutover. ADR-0006 strict-col set amended. |
