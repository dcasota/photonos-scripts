...
**Source file:** photonos-urlhealth-5.0_202605220934.prn

**Total packages analyzed:** 1113

**Total packages with issues:** 182
...
| 8 | Other warnings (VMware internal URL, unmaintained repo, etc.) | 45 | Low-Medium |
...
| 3 | ding-libs.spec | ding-libs | `https://github.com/SSSD/ding-libs/releases/download/0.6.2/ding-libs-0.6.2.tar.gz` | Source URL works but update detection found no newer version. May be correct or the version detection pattern does not match upstream release naming. Verify manually. |
...
| 28 | python-pywbem.spec | python-pywbem | `https://github.com/pywbem/pywbem/archive/refs/tags/1.4.1.tar.gz` | Source URL works but update detection found no newer version. May be correct or the version detection pattern does not match upstream release naming. Verify manually. |
| 29 | vulkan-headers.spec | vulkan-headers | `https://github.com/KhronosGroup/Vulkan-Headers/archive/refs/tags/v1.3.237.tar.gz` | Source URL works but update detection found no newer version. May be correct or the version detection pattern does not match upstream release naming. Verify manually. |
...
| 39 | mozjs.spec | mozjs | 151.0.1 | Warning: Manufacturer may changed version packaging format. | Upstream changed version/packaging format. Update the Source0 URL pattern in the spec to match the new release naming convention. |
| 40 | ncurses.spec | ncurses | 6.6.20260516 | Warning: Manufacturer may changed version packaging format. | Upstream changed version/packaging format. Update the Source0 URL pattern in the spec to match the new release naming convention. |
...
| 63 | qemu.spec | qemu | /9.1.2 | Warning: Manufacturer may changed version packaging format. | Upstream changed version/packaging format. Update the Source0 URL pattern in the spec to match the new release naming convention. |
...
| 34 | python-geomet.spec | python-geomet | 200 |  | Warning: Cannot detect correlating tags from the repo provided. | Tag detection failed. Check if upstream uses a different tagging convention. |
| 35 | python-lockfile.spec | python-lockfile | 200 |  | Warning: repo isn't maintained anymore. | Upstream repo is no longer maintained. Consider finding a fork, alternative, or mark package as archived. |
| 36 | python-psycopg2.spec | python-psycopg2 | pinned |  | vendor-pinned (subrelease 91) | Review warning and take appropriate action. |
| 37 | python-pyjsparser.spec | python-pyjsparser | 200 |  | Warning: Cannot detect correlating tags from the repo provided. | Tag detection failed. Check if upstream uses a different tagging convention. |
| 38 | python-terminaltables.spec | python-terminaltables | 200 | (same version) | Warning: repo isn't maintained anymore. | Upstream repo is no longer maintained. Consider finding a fork, alternative, or mark package as archived. |
| 39 | rpm.spec | rpm | pinned |  | vendor-pinned (subrelease 91) | Review warning and take appropriate action. |
| 40 | rubygem-async-io.spec | rubygem-async-io | 200 |  | Info: Source0 contains a VMware internal url address. | Source0 points to a VMware internal URL. Provide a public upstream URL if available. |
| 41 | runc.spec | runc | pinned |  | vendor-pinned (subrelease 91) | Review warning and take appropriate action. |
| 42 | shim-signed.spec | shim-signed | 200 |  | Info: Source0 contains a VMware internal url address. | Source0 points to a VMware internal URL. Provide a public upstream URL if available. |
| 43 | stalld.spec | stalld | pinned |  | vendor-pinned (subrelease 91) | Review warning and take appropriate action. |
| 44 | stig-hardening.spec | stig-hardening | 200 |  | Info: Source0 contains a VMware internal url address. | Source0 points to a VMware internal URL. Provide a public upstream URL if available. |
| 45 | strace.spec | strace | pinned |  | vendor-pinned (subrelease 91) | Review warning and take appropriate action. |
