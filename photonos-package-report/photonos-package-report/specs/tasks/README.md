...
|---|---|---|---|---|---|
| M111 | **PS-detection-success → col4="200" override** (mirrors PS L2531 clone-tag + L4408 generic-scrape urlhealth-set points). M102 unconditionally emitted the `substitution_unfinished` sentinel whenever modified Source0 carried `{`, even when a detection path had succeeded — flagging libaio + apache-tomcat9/10/11/-9 (11 spec×branch combinations, cat2 C-only). PS bypasses L2627 substitution_unfinished by setting `$urlhealth = "200"` in the clone-tag (L2531/L3869) and generic-scrape (L4408) success blocks. Track those two success paths with a new `health_overridden_200` flag (set on clone `git tag -l` success and on the generic-else scrape returning n>0); when subst_unfinished AND the flag is set, col4 emits the literal "200" instead of the sentinel. moz/gh/sf/ao/etc. branches intentionally do NOT set the flag (nss/dhcp/etc. preserve PS substitution_unfinished). ctest 13/13. | FRD-011 | 0001,0006 | 2531, 4408 | strict (col4 soft alignment + cat2 clears) |
| M110 | **github_api pagination** (FRD-006). `pr_github_api_names` fetched a single API page (default 30 entries). For `alternatives.spec` → `fedora-sysv/chkconfig` the first 100 tags are all CVS-style `r1-X-Y-Z`; the semver `1.33` sits at position 161 — after M109 dropped the `r1-` form, no semver candidates remained → empty col5. Add `per_page=100` and a page-N loop (page=1..20 safety bound; stop when a page returns <100 entries) so all tags are seen. Closes the alternatives col5 strict diff (PS=`1.33` ↔ C now also `1.33`). Page-1 failure still returns the original `-1`; pages 2+ failing keep what was already accumulated. ctest 13/13. | FRD-006 | 0001,0006 | n/a (C-side, PS handles via Invoke-RestMethod default Link-header pagination) | strict |
