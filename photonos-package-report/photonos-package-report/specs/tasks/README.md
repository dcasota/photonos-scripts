...
| M65 | **Option-D step 1: git-tag recorded-SHA pin** (record-replay determinism). The snapshot's `upstream-clones-manifest.tsv` records each upstream's HEAD SHA at PS-time; C now lists tags via `git tag --merged <sha>` (tags reachable from the recorded HEAD = the set PS saw) instead of `git tag -l` (live, includes tags pushed after the snapshot → temporal drift). New SHA-manifest loader in `clone.c` keyed by (photon-`<branch>`, repo) derived from clone_path; gated by `PR_UPSTREAM_SHA_MANIFEST` (set by the C workflow to the snapshot manifest); falls back to live tags per-repo on miss/force-push. Eliminates the git-tag temporal-drift source deterministically. | FRD-006 | 0009 | n/a (CI determinism) | strict |
