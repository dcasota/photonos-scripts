...
case "$CANISTER_MODE" in
  ...
  *)
    echo "[runPh4] ERROR: unsupported canister mode '$CANISTER_MODE'" 1>&2
    echo "[runPh4]        valid: prebuilt (default), build, acvp, kat" 1>&2
    exit 1
    ;;
esac
echo "[runPh4] Canister mode: $CANISTER_MODE"
...
" "$CANISTER_MODE" "$PKG_BUILD_OPTIONS" && \
    echo "[runPh4] Canister macros -> $PKG_BUILD_OPTIONS"
...
" 2>/dev/null && echo "[runPh4] pkg-build-options -> $PKG_BUILD_OPTIONS"
  fi
  if [ -f "$COMMON_CFG" ]; then
    POI_SET=$(python3 -c "
import json
