...
case "$IMG_TYPE" in
  ...
  *)
    echo "[runPh5_pinned90] ERROR: unsupported image type '$IMG_TYPE'" 1>&2
    echo "[runPh5_pinned90]        valid: iso (full), minimal-iso (default), basic-iso, rt-iso" 1>&2
    exit 1
    ;;
esac
...
" 2>/dev/null && echo "[runPh5_pinned90] Set base-commit to common HEAD: ${COMMON_HEAD}"
  ...
  if ! poi_image_ok; then
    ...
    if ! poi_image_ok; then
      echo "[runPh5_pinned90] ERROR: photon/installer:latest is missing or has no 'file'" 1>&2
      echo "[runPh5_pinned90]        binary. ISO assembly would fail in generateInitrd()" 1>&2
      echo "[runPh5_pinned90]        after every package has been rebuilt -- aborting now." 1>&2
      exit 1
    fi
  fi
...
" 2>/dev/null && echo "[runPh5_pinned90] Set poi-image to local photon/installer:latest"
  ...
  if [ -f "$PY3_SPEC" ] && ! grep -q 'PROFILE_TASK' "$PY3_SPEC"; then
    ...
    echo "[runPh5_pinned90] Fixed python3 spec: excluded test_generators from PGO training"
  fi
...
" 2>/dev/null | while IFS='|' read -r archive url sha; do
  ...
  echo "[runPh5_pinned90] Build stage: $BUILD_STAGE"
  ...
  # stage/iso/, but poi.py actually writes to $BUILD_STAGE/<IMG_NAME>/). That
  # is fixed below via iso_marker + iso_globs (maxdepth 2, -newer marker).
  ...
  for i in $(seq 1 10); do
    ...
    # stage/iso/photon-minimal-<ver>-<sha>.x86_64.iso -- NOT into
    # stage/ or stage/iso/ at the top level. Globbing only those two made the
    # loop miss a perfectly good, fully written ISO and burn every remaining
    # retry rebuilding it. Search one level deep in both stages, and take the
    ...
    if [ -n "$iso_found" ]; then
      echo "[runPh5_pinned90] Built ISO: $iso_found ($(du -h "$iso_found" | cut -f1))"
      ...
      if [ -n "$dup_found" ]; then
        echo "[runPh5_pinned90] Identical ISO already present at $dup_found (sha256 $iso_sha) -- not moving/overwriting; nothing left to do."
        exit 0
      fi
      ...
      if [ -e "$dest" ]; then
        ...
        echo "[runPh5_pinned90] $OUTPUT_DIR/$(basename "$iso_found") already exists with different content; delivering new ISO as $(basename "$dest") instead"
      fi
      if sudo mv "$iso_found" "$dest"; then
        echo "[runPh5_pinned90] Moved ISO to $dest"
        exit 0
      fi
      echo "[runPh5_pinned90] ERROR: could not move ISO to $dest" 1>&2
      echo "[runPh5_pinned90]        It is still at: $iso_found" 1>&2
      exit 1
    fi
    ...
    echo "[runPh5_pinned90] Attempt $i: no ISO produced (make exit=$make_rc, $progress file(s) touched since marker)"
    if [ "$i" -gt 1 ] && [ "$make_rc" = "$prev_make_rc" ] && [ "$progress" = "0" ] && [ "$prev_progress" = "0" ]; then
      echo "[runPh5_pinned90] ERROR: attempt $i failed identically to attempt $((i - 1)) (same make exit code, zero new output both times)." 1>&2
      echo "[runPh5_pinned90]        This looks like a deterministic failure, not a flaky one -- further retries would just reproduce it." 1>&2
      echo "[runPh5_pinned90]        Stopping after $i/10 attempts. Fix the underlying build error, then re-run." 1>&2
      exit 1
    fi
    prev_make_rc=$make_rc
    prev_progress=$progress
  done
  echo "[runPh5_pinned90] ERROR: exhausted all 10 attempts without producing an ISO" 1>&2
  exit 1
else
  ...
  echo "[runPh5_pinned90] ERROR: no network (ping www.google.ch failed); the build" 1>&2
  echo "[runPh5_pinned90]        needs to fetch sources and was not started." 1>&2
  exit 1
fi
