...
echo "[runPh7-2-7] wrapper v9 + COMMON_DIR default + worktree_now same-shell + SpecData compat cleanup"
...
git_sync_quiet() {
  repo="$1"
  [ -d "$repo/.git" ] || return 0
  echo "[runPh7-2-7] git sync (no editor) $repo"
  ...
  if git -C "$repo" pull --ff-only --no-edit --quiet 2>/dev/null; then
    echo "[runPh7-2-7] $repo fast-forwarded"
    return 0
  fi
  if git -C "$repo" pull --no-rebase --no-edit --quiet 2>/dev/null; then
    echo "[runPh7-2-7] $repo merged with --no-edit (ort)"
    return 0
  fi
  echo "[runPh7-2-7] $repo: leave tree as-is (fetch/pull not clean)"
}
...
RELEASE_BRANCH="${RELEASE_BRANCH:-experimental/linux-7.2.7}"
...
for spec in Path("SPECS").rglob("*.spec"):
    ...
    nt = t.replace("# 7.2.7 always build STIG dep\n# %global build_if", "%global build_if")
    nt = re.sub(r'(?m)^# (%global build_if\b)', r'\1', nt)
    if nt != t:
        spec.write_text(nt)
        n += 1
print("[runPh7-2-7] re-gated build_if in", n, "spec(s)")
PY
}
...
if nt == t:
    print(f"[runPh7-2-7] {p}: no SpecData compat leftovers")
    raise SystemExit(0)
...
sync_cve_include() {
  ...
  if git remote get-url origin >/dev/null 2>&1; then
    ...
    if git show "origin/${RELEASE_BRANCH}:${inc}" >/dev/null 2>&1; then
      ...
      echo "[runPh7-2-7] refreshed $inc from origin/${RELEASE_BRANCH}"
    fi
  fi
  if [ ! -f "$inc" ] || grep -qE '^[[:space:]]*Patch[0-9]+' "$inc"; then
    echo "[runPh7-2-7] $inc still lists Patch lines (or missing); writing empty include"
    write_empty_cve_inc "$inc"
  fi
  if grep -qE '^[[:space:]]*Patch[0-9]+' "$inc"; then
    echo "[runPh7-2-7] ERROR: $inc still has Patch lines after cleanup" 1>&2
...
# which already apply on 7.2.7. Comment EVERY %autopatch line (no live
...
disable_unrebased_ranges() {
  spec="$1"
  [ -f "$spec" ] || return 0
  sed -i 's/^%autopatch /# unrebased autopatch skipped for 7.2.7: /' "$spec"
  if grep -q '%setup -q -n linux-%{version}' "$spec"; then
    awk '
      { print }
      $0 ~ /%setup -q -n linux-%\{version\}/ && !done {
  ...
  echo "[runPh7-2-7] $spec: only %autopatch -m0 -M1 (Patch0+Patch1)"
}
...
drop_skipped_patches() {
  ...
  echo "$SKIP_PATCHES" | while IFS= read -r pat; do
    ...
    if grep -q "$pat" "$spec"; then
      sed -i "/$pat/d" "$spec"
      echo "[runPh7-2-7] dropped $pat from $spec"
    fi
...
inject_config_merge() {
  ...
  needle="$2"
  [ -f "$spec" ] || return 0
  echo "[runPh7-2-7] config merge on $spec ..."
...
start = next((i for i, l in enumerate(lines) if "7.2.7 config merge" in l), None)
...
skip_oot_modules() {
  spec="$1"
  [ -f "$spec" ] || return 0
  echo "[runPh7-2-7] skip OOT modules in $spec ..."
  python3 - "$spec" << 'PY'
import sys
from pathlib import Path
p = Path(sys.argv[1])
lines = p.read_text().splitlines(keepends=True)
if any("7.2.7 skip OOT" in l for l in lines):
    print(f"[runPh7-2-7] {p}: OOT modules already skipped")
    raise SystemExit(0)
...
if count:
    ...
    print(f"[runPh7-2-7] {p}: wrapped {count} ENA/EFA/viomem block(s)")
else:
    print(f"[runPh7-2-7] {p}: no ENA/EFA/viomem build blocks found")
PY
}
...
add_buildrequires() {
  ...
  if grep -qE "^BuildRequires:[[:space:]]*${pkg}([[:space:]]|\$)" "$spec"; then
    echo "[runPh7-2-7] $spec already BuildRequires: $pkg"
    return 0
  fi
  python3 - "$spec" "$pkg" << 'PY'
import sys
...
for i, l in enumerate(lines):
    if l.startswith("BuildRequires:"):
        ...
        print(f"[runPh7-2-7] {p}: added BuildRequires: {pkg}")
        raise SystemExit(0)
p.write_text(ins + "".join(lines))
print(f"[runPh7-2-7] {p}: prepended BuildRequires: {pkg}")
PY
}
...
pin_cpupower_nls() {
  ...
  if grep -q 'cpupower_install NLS=' "$spec"; then
    echo "[runPh7-2-7] $spec already has cpupower NLS="
    return 0
  fi
  if grep -q 'cpupower_install' "$spec"; then
    sed -i 's/cpupower_install/cpupower_install NLS=/' "$spec"
    echo "[runPh7-2-7] $spec: cpupower_install NLS= (skip missing .gmo)"
  fi
}
...
pin_rust_probe() {
  ...
  echo "[runPh7-2-7] rust: restored upstream spec (no cxxwrap, no channel=dev)"
  python3 - "$spec" << 'PY'
from pathlib import Path
...
for l in lines:
    ...
    out.append(l)
text = "".join(out)
hook = "# 7.2.7 rust locale only\nexport LANG=C LC_ALL=C LC_MESSAGES=C GCC_COLORS=\n"
if hook not in text:
    text = text.replace("%build\n", "%build\n" + hook, 1)
p.write_text(text)
print(f"[runPh7-2-7] {p}: upstream rust.spec + LANG=C only")
PY
}
...
pin_linux_files() {
  spec="$1"
  [ -f "$spec" ] || return 0
  echo "[runPh7-2-7] %files cleanup on $spec ..."
  python3 - "$spec" << 'PY'
from pathlib import Path
p = Path(__import__("sys").argv[1])
text = p.read_text()
...
text = text.replace("# {_datadir}/perf-core  # 7.2.7 perf install path changed", "%{_datadir}/perf-core")
...
if "7.2.7 mkdir perf-core" not in text:
    hook = (
        "# 7.2.7 mkdir perf-core\n"
...
if text != orig:
    p.write_text(text)
    print(f"[runPh7-2-7] {p}: docs glob restored, perf-core dirs ensured")
else:
    print(f"[runPh7-2-7] {p}: %files already consistent")
PY
}
...
pin_rust_skip_docs() {
  ...
  [ -f "$spec" ] || spec=$(find SPECS -name rust.spec 2>/dev/null | head -n1)
  [ -n "$spec" ] && [ -f "$spec" ] || return 0
  echo "[runPh7-2-7] rust docs=false via configure (strip bootstrap shim) ..."
  python3 - "$spec" << 'PY'
from pathlib import Path
p = Path(__import__("sys").argv[1])
lines = p.read_text().splitlines(keepends=True)
out, skip = [], False
for l in lines:
    if l.startswith("# 7.2.7 rust docs=false") or l.startswith("_rust_disable_docs"):
        skip = True
        continue
    if skip:
...
text = text.replace(
    "# 7.2.7 rust-doc files: man pages absent when docs=false\n# {_mandir}/man1/*",
    "%{_mandir}/man1/*",
)
...
restore = (
    "# 7.2.7 restore bootstrap.py if a prior shim moved it\n"
...
if "prior shim moved it" not in text:
    ...
    for i, l in enumerate(lines):
        if l.startswith("%build"):
            ...
            break
p.write_text(text)
print(f"[runPh7-2-7] {p}: configure --set build.docs=false; bootstrap shim removed")
PY
}
pin_rust_skip_docs
...
sanitizer = (
    "\n# 7.2.7 postgres config.status sanitizer\n"
    "if grep -qE '^0a[0-9]' config.status 2>/dev/null; then\n"
    "  echo '[runPh7-2-7] stripping ed cache blob only (keep Makefile rules)'\n"
    ...
    "  echo '[runPh7-2-7] re-run config.status to write Makefile.global'\n"
...
if "# 7.2.7 postgres config.status sanitizer" not in text:
    text = text.replace("%make_build world", sanitizer + "%make_build world")
p.write_text(text)
print(f"[runPh7-2-7] {p}: cache-file=/dev/null + safe sanitizer")
PY
}
pin_postgres_config
...
pin_svn_nodebug() {
  ...
  [ -n "$spec" ] && [ -f "$spec" ] || return 0
  if grep -q '7.2.7 exclude debug' "$spec"; then
    echo "[runPh7-2-7] $spec already excludes /usr/lib/debug"
    return 0
  fi
  python3 - "$spec" << 'PY'
from pathlib import Path
...
for line in text.splitlines(keepends=True):
    ...
    if line.startswith("%files"):
        out.append(ex)
p.write_text("".join(out))
print(f"[runPh7-2-7] {p}: exclude /usr/lib/debug from %files")
PY
}
pin_svn_nodebug
...
pin_docker_devnull() {
  ...
  [ -n "$spec" ] && [ -f "$spec" ] || return 0
  if grep -q '7.2.7 /dev/null' "$spec"; then
    echo "[runPh7-2-7] $spec already ensures /dev/null"
  else
    python3 - "$spec" << 'PY'
from pathlib import Path
p = Path(__import__("sys").argv[1])
...
if "%build\n" in text:
    ...
    print(f"[runPh7-2-7] {p}: mknod /dev/null in %build")
PY
  fi
}
pin_docker_devnull
...
fix_null_node() {
  path="$1"
  [ -c "$path" ] && return 0
  echo "[runPh7-2-7] $path is not a char device; recreating c 1 3"
  rm -f "$path" 2>/dev/null || true
  if ! mknod -m 666 "$path" c 1 3; then
    echo "[runPh7-2-7] mknod $path failed (need CAP_MKNOD). docker POI will break." 1>&2
    return 1
  fi
  [ -c "$path" ] && return 0
  echo "[runPh7-2-7] ERROR: $path still not char device after mknod" 1>&2
  ls -l "$path" 1>&2 || true
  return 1
}
...
ensure_sandbox_dev() {
  ...
  if [ -c /dev/null ]; then
    echo "[runPh7-2-7] host /dev/null is a char device (docker POI ok)"
  else
    echo "[runPh7-2-7] WARNING: host /dev/null is still not c 1 3 — ISO docker run will fail" 1>&2
  fi
}
ensure_sandbox_dev
...
pin_apparmor_perl() {
  ...
  [ -n "$spec" ] && [ -f "$spec" ] || return 0
  if grep -q '7.2.7 apparmor perl paths' "$spec"; then
    echo "[runPh7-2-7] $spec already has perl path fallback"
    return 0
  fi
  python3 - "$spec" << 'PY'
from pathlib import Path
...
if header not in text:
    text = text.replace("%build\n", header + "%build\n", 1)
install = (
    "\n# 7.2.7 apparmor perl stub if bindings missing\n"
...
p.write_text(text)
print(f"[runPh7-2-7] {p}: perl_vendorarch fallback + stub files")
PY
}
pin_apparmor_perl
...
if [ -f SPECS/linux/linux.spec ]; then
  ...
  echo "[runPh7-2-7] linux.spec not in $PWD (helper pre-pass?); skipping version assert"
fi
...
if nt != t:
    p.write_text(nt)
    print(f"[runPh7-2-7] {p}: spec-checker whitespace normalized")
PY
  done
}
pin_spec_lint
...
chmod +x "$GEN" "$PINOUT"
if [ ! -c /dev/null ]; then
  echo "[runPh7-2-7] repairing host /dev/null before ISO docker"
