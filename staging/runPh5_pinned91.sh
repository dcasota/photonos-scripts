...
case "$CANISTER_MODE" in
  ...
  *)
    echo "[runPh5_pinned91] ERROR: unsupported canister mode '$CANISTER_MODE'" 1>&2
    echo "[runPh5_pinned91]        valid: prebuilt (default), build, acvp, kat" 1>&2
    exit 1
    ;;
esac
echo "[runPh5_pinned91] Canister mode: $CANISTER_MODE"
...
" "$CANISTER_MODE" "$PKG_BUILD_OPTIONS" && \
    echo "[runPh5_pinned91] Canister macros -> $PKG_BUILD_OPTIONS"
...
with open('$COMMON_CFG', 'w') as f:
    json.dump(cfg, f, indent=4)
" 2>/dev/null && echo "[runPh5_pinned91] pkg-build-options -> $PKG_BUILD_OPTIONS"
  fi
  if [ -f "$COMMON_CFG" ]; then
    POI_SET=$(python3 -c "
import json
