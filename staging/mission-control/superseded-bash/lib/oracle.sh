...
mc_oracle_install() {
    ...
    # Two independent proofs, either sufficient. The serial marker only appears
    # if the INSTALLED system has a serial console; a target whose grub edit did
    # not take is silent here even though it booted perfectly well. In that case
    # the guest answering on the network is the stronger evidence, so accept it.
    local booted=no
    [ "$parts" -gt 0 ] && booted=yes
    [ "${MC_INSTALL_RESULT:-}" = installed ] && booted=yes
    mc_expect install.booted_from_disk "-" "yes" "$booted" \
