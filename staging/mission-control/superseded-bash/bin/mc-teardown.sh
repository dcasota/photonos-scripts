...
if [ "$PURGE" -eq 1 ]; then
    ...
    find "$DIR" -name "*.stashed-*" -delete >/dev/null 2>&1 || true
    mc_log "purged $old stashed file(s) to reclaim space"
    # --recreate stashes the WHOLE VM directory as a sibling
    # mc-<perm>.stashed-<ts>/, which the file-level purge above never sees.
    # Nine of them accumulated during one debugging session, and a disk that
    # fills is indistinguishable from a hypervisor that refuses to start a VM.
    # in-place stashes created by mc-create-vm --recreate
    inplace=0
    for st in "$DIR"/stash-*; do
        [ -d "$st" ] || continue
        find "$st" -type f -delete >/dev/null 2>&1 || true
        rmdir "$st" 2>/dev/null && inplace=$((inplace+1))
    done
    [ "$inplace" -gt 0 ] && mc_log "purged $inplace in-place stash director(ies)"
    # legacy sibling directories from the old move-the-whole-dir behaviour
    dirs=0
    for st in "${MC_VM_ROOT_WSL}/${VM}".stashed-*; do
        [ -d "$st" ] || continue
        find "$st" -type f -delete >/dev/null 2>&1 || true
        rmdir "$st" 2>/dev/null && dirs=$((dirs+1))
    done
    mc_log "purged $dirs stashed VM directory(ies)"
fi
