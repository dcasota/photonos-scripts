...
read -r _ ISO_TYPE POI STIG FS MODE VARIANT DOC EXPECT <<EOF
...
mc_result_init "$PERM"
HARVEST="$MC_RESULTS_DIR/$PERM/logs"; mkdir -p "$HARVEST"
...
# --- media ---------------------------------------------------------------
ISO_LINK="$MC_ISO_CACHE/${ISO_TYPE}-poi${POI}-prebuilt/photon.iso"
if [ -f "$ISO_LINK" ]; then
